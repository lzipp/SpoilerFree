...
var contact_message_to_background={'tag': 'load_to_content'}
chrome.runtime.sendMessage({greeting: contact_message_to_background}, function(response) {
...
var add_observer_listener=function(){
      ...
      config_mutation = { attributes: false, childList: true, characterData: true, subtree: true}
...
var word_check=function(){
  ...
  for (i=0; i<blocked_list.length; i +=1){
    ...
    if (all_content.toLowerCase().indexOf(blocked_word.toLowerCase()) != -1){
        return {"found":true,"blocked_word":blocked_word};
    } 
  }
          return {"found":false,"blocked_word":""};
};
...
function cover_words(){
  ...
  for (i_count=0; i_count<blocked_list.length; i_count +=1){
      ...
      if (phrase_node.length>0){
        ...
        new_buttons.click(function(ev){spoil_btn_click_func(this,ev)});  //Sets the click function on all spoiler buttons
        new_buttons.data("placed","true");
  }
}
}// end of cover_blocked_wordds function
